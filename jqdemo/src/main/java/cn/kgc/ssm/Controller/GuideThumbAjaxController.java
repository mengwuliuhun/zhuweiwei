...
@RestController
public class GuideThumbAjaxController {
    @Resource
    GuideThumbService service;

    @RequestMapping(value = "/changeGuideThumb")
    public JSONData insertCoolItem(GuideThumb thumb, HttpServletRequest request) {
        ...
        try {
            int ret = service.changeByUserAndGuide(thumb);
            if (ret == 1) {
