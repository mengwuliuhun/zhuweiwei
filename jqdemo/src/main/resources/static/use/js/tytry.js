$(function () {
    ...
    $.getJSON("../tryItemVo_ajax", {category: "体验师专享", type: "try"}, function (data) {
        var self = $(".comMore");
        self.addClass("loading").html("正在加载中");
        var ret = data.data;
        self.data("pno", 2).data("psize", 4);
        var allLen = ret.navigateLastPage;//获取json长度
        var list = ret.list;
    ...
    $(".comMore").click(function () {
        ...
        var self = $(this);
        self.addClass("loading").html("正在加载中");
        $.getJSON("../tryItemVo_ajax", {category: "体验师专享", type: "try", pno: pno, psize: psize}, function (data) {
            var ret = data.data;
